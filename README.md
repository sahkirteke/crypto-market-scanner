...
If Supabase transaction pooling is used, the JDBC URL may differ from the direct connection or session pooler URL. Keep `sslmode=require` enabled for Supabase connections. When the `db` or `manual-scanner-db` profile is active without `SUPABASE_DB_URL`, the application falls back to `jdbc:postgresql://aws-1-eu-central-1.pooler.supabase.com:6543/postgres?prepareThreshold=0&sslmode=require` rather than passing an unresolved placeholder to HikariCP.
