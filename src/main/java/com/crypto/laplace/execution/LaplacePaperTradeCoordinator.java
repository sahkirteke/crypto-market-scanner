...
private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 public void onSignal(LaplaceSignalResult signal,boolean entryUniverseSnapshotContainsSymbol){
  ...
  ReentrantLock lock=locks.computeIfAbsent(signal.symbol(),s->new ReentrantLock());lock.lock();try {
   ...
   if(riskyDirectionBlocked){log.info("LAPLACE_RISKY_DIRECTION_BLOCK action=SKIP reason=RISKY_DIRECTION_ALREADY_BLOCKED symbol={} rawSignal={} signalCandleCloseTime={}",signal.symbol(),signal.entrySignal(),signal.signalCandleCloseTime());return;}
 ...
 private void coordinate(LaplaceSignalResult signal,boolean poolActiveAfterTransition,PositionSide effective,boolean entryUniverseSnapshotContainsSymbol,Instant eligibilityEvaluatedAt){
  ...
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(riskyEntryFilter.isRisky(signal)){blockRiskyDirection(signal);writer.riskyEntrySkipped(signal.symbol(),signal.signalCandleCloseTime(),effective);log.info("LAPLACE_RISKY_ENTRY_FILTER action=SKIP reason=RISKY_ENTRY_FILTER riskyDirectionBlocked=true symbol={} rawSignal={} effectiveExecutionSide={} atrPercentage={} previousRawTakerImbalance={} riskyEntry=true signalCandleCloseTime={}",signal.symbol(),signal.entrySignal(),effective,signal.atrPercentage(),signal.previousRawTakerImbalance(),signal.signalCandleCloseTime());return;}metrics.entryAttempts.incrementAndGet();if(!currentlyEligible){
  ...
  boolean riskyReversalEntry=riskyEntryFilter.isRisky(signal);if(riskyReversalEntry){blockRiskyDirection(signal);writer.riskyEntrySkipped(signal.symbol(),signal.signalCandleCloseTime(),effective);log.info("LAPLACE_RISKY_ENTRY_FILTER action=SKIP reason=RISKY_ENTRY_FILTER riskyDirectionBlocked=true symbol={} rawSignal={} effectiveExecutionSide={} atrPercentage={} previousRawTakerImbalance={} riskyEntry=true signalCandleCloseTime={}",signal.symbol(),signal.entrySignal(),effective,signal.atrPercentage(),signal.previousRawTakerImbalance(),signal.signalCandleCloseTime());}
