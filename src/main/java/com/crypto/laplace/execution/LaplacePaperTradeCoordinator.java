...
public class LaplacePaperTradeCoordinator {
 ...
 private void coordinate(LaplaceSignalResult signal,boolean inEntryUniverse,PositionSide effective){
  ...
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(!inEntryUniverse){failure(signal,"FLAT","ENTRY","SYMBOL_OUTSIDE_ENTRY_UNIVERSE",null,false);return;}if(skipHighAtr30m(signal)||skipHighRiskLong(signal,effective))return;try{execution.open(signal,effective,null,"FLAT");writer.drain();}catch(RuntimeException e){failure(signal,"FLAT","ENTRY",reason(e,"ENTRY_EXECUTION_FAILED"),e,true);}return;}
  ...
  PositionSide reversalTarget=mapRawSignalToExecutionSide(signal.strongReversalSignal());if(reversalTarget==null||reversalTarget!=effective)return;if(skipHighAtr30m(signal)||skipHighRiskLong(signal,effective))return;
 ...
 private boolean skipHighAtr30m(LaplaceSignalResult signal){if(signal.signalCandleClose()<=0)return false;double atrPercent30m=signal.currentAtr14()/signal.signalCandleClose()*100.0;if(atrPercent30m<=0.40)return false;log.info("LAPLACE_ENTRY_SKIPPED_ATR_30M symbol={} signalTime={} currentAtr14_30m={} signalCandleClose={} atrPercent30m={} threshold=0.40",signal.symbol(),signal.signalCandleCloseTime(),signal.currentAtr14(),signal.signalCandleClose(),atrPercent30m);return true;}
