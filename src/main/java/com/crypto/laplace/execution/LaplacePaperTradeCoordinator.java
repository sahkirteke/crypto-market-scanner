...
import com.crypto.laplace.persistence.*;import com.crypto.laplace.service.LaplaceSymbolBlockService;
...
public class LaplacePaperTradeCoordinator {
 private final LaplaceStrategyProperties config;private final LaplacePaperPositionRepository positions;private final LaplacePaperExecutionService execution;private final LaplaceTradeJsonlWriter writer;private final LaplaceSymbolBlockService blocks;private final ConcurrentHashMap<String,ReentrantLock> locks=new ConcurrentHashMap<>();private final ConcurrentHashMap<String,LaplaceSignal> rawStates=new ConcurrentHashMap<>();private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 private void coordinate(LaplaceSignalResult signal,boolean inEntryUniverse,PositionSide effective){
  ...
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(!inEntryUniverse){failure(signal,"FLAT","ENTRY","SYMBOL_OUTSIDE_ENTRY_UNIVERSE",null,false);return;}if(blocks.isBlocked(signal.symbol())){failure(signal,"FLAT","ENTRY","SYMBOL_STOP_LOSS_BLOCKED",null,false);return;}try{execution.open(signal,effective,null,"FLAT");writer.drain();}catch(RuntimeException e){failure(signal,"FLAT","ENTRY",reason(e,"ENTRY_EXECUTION_FAILED"),e,true);}return;}
  ...
  try{execution.reverse(signal,p,effective,inEntryUniverse&&!blocks.isBlocked(signal.symbol()));writer.drain();}catch(RuntimeException e){failure(signal,p.getSide().name(),"REVERSAL",reason(e,"REVERSAL_CLOSE_FAILED"),e,true);}
 }
