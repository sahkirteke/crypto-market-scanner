...
public class LaplacePaperTradeCoordinator {
 private final LaplaceStrategyProperties config;private final LaplacePaperPositionRepository positions;private final LaplacePaperExecutionService execution;private final LaplaceTradeJsonlWriter writer;private final LaplaceEntryDecisionService entryDecisionService;private final LaplaceDiagnosticLogService diagnostics;private final ConcurrentHashMap<String,ReentrantLock> locks=new ConcurrentHashMap<>();private final ConcurrentHashMap<String,LaplaceSignal> rawStates=new ConcurrentHashMap<>();private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 private void coordinate(LaplaceSignalResult signal,boolean inEntryUniverse,PositionSide effective){
  ...
  try{var outcome=execution.reverse(signal,p,effective,false);writer.drain();if(!inEntryUniverse)return;var decision=entryDecisionService.evaluate(signal,effective);diagnostics.entryDecision(signal,decision);if(decision.allowed())execution.open(signal,effective,outcome.reversalId(),p.getSide().name(),decision);writer.drain();}catch(RuntimeException e){failure(signal,p.getSide().name(),"REVERSAL",reason(e,"REVERSAL_CLOSE_FAILED"),e,true);}
 }
