...
public class LaplacePaperTradeCoordinator {
 ...
 private void coordinate(LaplaceSignalResult signal,boolean inEntryUniverse,PositionSide effective){
  ...
  try{management.catchUp(p.getId(),signal.signalCandleCloseTime());List<LaplacePaperPositionEntity> stillOpen=positions.findByStrategyAndSymbolAndStatus(LaplacePaperExecutionService.STRATEGY,signal.symbol(),LaplacePositionStatus.OPEN);String reversalId=null;if(!stillOpen.isEmpty()){var outcome=execution.closeForOppositeSignal(signal,stillOpen.getFirst(),effective);reversalId=outcome.reversalId();writer.drain();}if(!inEntryUniverse)return;var decision=entryDecisionService.evaluate(signal,effective);diagnostics.entryDecision(signal,decision);if(decision.allowed())execution.open(signal,effective,reversalId,p.getSide().name(),decision);writer.drain();}catch(RuntimeException e){failure(signal,p.getSide().name(),"REVERSAL",reason(e,"REVERSAL_CLOSE_FAILED"),e,true);}
 }
