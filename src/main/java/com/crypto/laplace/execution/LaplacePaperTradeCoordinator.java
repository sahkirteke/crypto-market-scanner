...
public class LaplacePaperTradeCoordinator {
 ...
 private boolean entryBlocked(LaplaceSignalResult signal,PositionSide effective){var cooldown=symbolCooldown.active(signal.symbol());if(cooldown.isEmpty())return false;double atrPercentage=signal.currentAtr14()/signal.signalCandleClose()*100.0;double slopeStrength=Math.abs(signal.currentNormalizedSlope());var active=cooldown.get();diagnostics.trueSymbolStopLossCooldown(signal,effective,atrPercentage,slopeStrength,active,symbolCooldown.remainingSeconds(active));log.info("LAPLACE_ENTRY_BLOCKED reason=TRUE_SYMBOL_STOP_LOSS_COOLDOWN symbol={} cooldownUntil={} signalTime={}",signal.symbol(),active.cooldownUntil(),signal.signalCandleCloseTime());return true;}
