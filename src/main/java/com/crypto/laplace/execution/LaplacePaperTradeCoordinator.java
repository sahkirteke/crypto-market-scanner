...
private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 private final ConcurrentHashMap<String,LaplaceSignal> pendingRiskyEntries=new ConcurrentHashMap<>();
 ...
 public void onSignal(LaplaceSignalResult signal,boolean entryUniverseSnapshotContainsSymbol){
  ...
  ReentrantLock lock=locks.computeIfAbsent(signal.symbol(),s->new ReentrantLock());lock.lock();try {
   ...
   boolean pendingRisky=isPendingRiskyEntry(signal.symbol(),signal.entrySignal());
   if(fresh&&!pendingRisky&&hasAnyPendingRiskyEntry(signal.symbol()))clearPendingRiskyEntry(signal.symbol());
   ...
   if((fresh||pendingRisky)&&oppositeAllowed)coordinate(signal,poolActive,effective,entryUniverseSnapshotContainsSymbol,Instant.now());
  }finally{lock.unlock();}
 }
 ...
 private void coordinate(LaplaceSignalResult signal,boolean poolActiveAfterTransition,PositionSide effective,boolean entryUniverseSnapshotContainsSymbol,Instant eligibilityEvaluatedAt){
  ...
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(riskyEntryFilter.isRisky(signal)){recordPendingRiskyEntry(signal);writer.riskyEntrySkipped(signal.symbol(),signal.signalCandleCloseTime(),effective);log.info("LAPLACE_RISKY_ENTRY_FILTER action=SKIP reason=RISKY_ENTRY_FILTER entryStateConsumed=false entryRemainsArmed=true symbol={} rawSignal={} effectiveExecutionSide={} atrPercentage={} previousRawTakerImbalance={} riskyEntry=true lastObservedSignal={} signalCandleCloseTime={}",signal.symbol(),signal.entrySignal(),effective,signal.atrPercentage(),signal.previousRawTakerImbalance(),signal.entrySignal(),signal.signalCandleCloseTime());return;}metrics.entryAttempts.incrementAndGet();if(!currentlyEligible){
  ...
  boolean riskyReversalEntry=riskyEntryFilter.isRisky(signal);if(riskyReversalEntry){recordPendingRiskyEntry(signal);writer.riskyEntrySkipped(signal.symbol(),signal.signalCandleCloseTime(),effective);log.info("LAPLACE_RISKY_ENTRY_FILTER action=SKIP reason=RISKY_ENTRY_FILTER entryStateConsumed=false entryRemainsArmed=true symbol={} rawSignal={} effectiveExecutionSide={} atrPercentage={} previousRawTakerImbalance={} riskyEntry=true lastObservedSignal={} signalCandleCloseTime={}",signal.symbol(),signal.entrySignal(),effective,signal.atrPercentage(),signal.previousRawTakerImbalance(),signal.entrySignal(),signal.signalCandleCloseTime());}
  ...
  try{execution.reverse(signal,p,effective,reversalTargetAllowed);if(reversalTargetAllowed)clearPendingRiskyEntry(signal.symbol());writer.drain();}catch(RuntimeException e){failure(signal,p.getSide().name(),"REVERSAL",reason(e,"REVERSAL_CLOSE_FAILED"),e,true);}
 }
 ...
 private boolean isPendingRiskyEntry(String symbol,LaplaceSignal raw){return pendingRiskyEntries.get(symbol)==raw||(coinPool!=null&&coinPool.hasPendingRiskyEntry(symbol,raw.name()));}
 private boolean hasAnyPendingRiskyEntry(String symbol){return pendingRiskyEntries.containsKey(symbol)||(coinPool!=null&&coinPool.hasAnyPendingRiskyEntry(symbol));}
 private void recordPendingRiskyEntry(LaplaceSignalResult signal){pendingRiskyEntries.put(signal.symbol(),signal.entrySignal());if(coinPool!=null)coinPool.recordPendingRiskyEntry(signal.symbol(),signal.entrySignal().name(),signal.signalCandleCloseTime());}
 private void clearPendingRiskyEntry(String symbol){pendingRiskyEntries.remove(symbol);if(coinPool!=null)coinPool.clearPendingRiskyEntry(symbol);}
