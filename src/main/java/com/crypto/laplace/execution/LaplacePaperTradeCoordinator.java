...
private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 public void onSignal(LaplaceSignalResult signal,boolean inEntryUniverse){
  ...
  ReentrantLock lock=locks.computeIfAbsent(signal.symbol(),s->new ReentrantLock());lock.lock();try {boolean poolActive=coinPool==null || coinPool.observeTrend(signal.symbol(),trend(signal.entrySignal()));boolean oppositeAllowed=oppositeSignalAllowed(signal.symbol(),signal.entrySignal());if(config.getLaplace().getStopReentryPolicy()==StopReentryPolicy.COOLDOWN&&stopCooldowns.getOrDefault(signal.symbol(),Instant.EPOCH).isBefore(Instant.now())){rawStates.put(signal.symbol(),LaplaceSignal.NONE);stopCooldowns.remove(signal.symbol());} LaplaceSignal previous=rawStates.put(signal.symbol(),signal.entrySignal());if(previous==null){log.info("LAPLACE_STARTUP_RAW_BASELINE_INITIALIZED symbol={} startupRawSignalState={}",signal.symbol(),signal.entrySignal());return;}boolean fresh=previous!=signal.entrySignal();PositionSide effective=mapRawSignalToExecutionSide(signal.entrySignal());log.info("LAPLACE_FRESH_SIGNAL rawSignal={} effectiveExecutionSide={} previousRawState={} currentRawState={} isFreshSignal={} currentPosition={} requestedAction={}",signal.entrySignal(),effective,previous,signal.entrySignal(),fresh,current(signal.symbol()),fresh&&effective!=null?"EVALUATE":"NONE");if(fresh&&oppositeAllowed)coordinate(signal,poolActive && inEntryUniverse,effective);}finally{lock.unlock();}
 }
 ...
 private void coordinate(LaplaceSignalResult signal,boolean inEntryUniverse,PositionSide effective){
  ...
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(!inEntryUniverse || (coinPool!=null && !coinPool.canOpen(signal.symbol()))){failure(signal,"FLAT","ENTRY","SYMBOL_OUTSIDE_ENTRY_UNIVERSE",null,false);return;}try{execution.open(signal,effective,null,"FLAT");writer.drain();}catch(RuntimeException e){failure(signal,"FLAT","ENTRY",reason(e,"ENTRY_EXECUTION_FAILED"),e,true);}return;}
