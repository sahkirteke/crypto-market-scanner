...
private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 private void coordinate(LaplaceSignalResult signal,boolean poolActiveAfterTransition,PositionSide effective,boolean entryUniverseSnapshotContainsSymbol,Instant eligibilityEvaluatedAt){
  ...
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(riskyEntryFilter.isRisky(signal)){recordPendingRiskyEntry(signal);log.info("LAPLACE_RISKY_ENTRY_FILTER action=SKIP reason=RISKY_ENTRY_FILTER entryStateConsumed=false entryRemainsArmed=true symbol={} rawSignal={} effectiveExecutionSide={} atrPercentage={} previousRawTakerImbalance={} riskyEntry=true lastObservedSignal={} signalCandleCloseTime={}",signal.symbol(),signal.entrySignal(),effective,signal.atrPercentage(),signal.previousRawTakerImbalance(),signal.entrySignal(),signal.signalCandleCloseTime());return;}metrics.entryAttempts.incrementAndGet();if(!currentlyEligible){
  ...
  boolean riskyReversalEntry=riskyEntryFilter.isRisky(signal);if(riskyReversalEntry){recordPendingRiskyEntry(signal);log.info("LAPLACE_RISKY_ENTRY_FILTER action=SKIP reason=RISKY_ENTRY_FILTER entryStateConsumed=false entryRemainsArmed=true symbol={} rawSignal={} effectiveExecutionSide={} atrPercentage={} previousRawTakerImbalance={} riskyEntry=true lastObservedSignal={} signalCandleCloseTime={}",signal.symbol(),signal.entrySignal(),effective,signal.atrPercentage(),signal.previousRawTakerImbalance(),signal.entrySignal(),signal.signalCandleCloseTime());}
