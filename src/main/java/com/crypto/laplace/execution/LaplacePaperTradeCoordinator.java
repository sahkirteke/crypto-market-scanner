...
public class LaplacePaperTradeCoordinator {
 ...
 private PositionSide catchUpAndLongFastExit(LaplaceSignalResult signal){
  List<LaplacePaperPositionEntity> open=positions.findByStrategyAndSymbolAndStatus(LaplacePaperExecutionService.STRATEGY,signal.symbol(),LaplacePositionStatus.OPEN);if(open.size()!=1)return null;management.catchUp(open.getFirst().getId(),signal.signalCandleCloseTime());open=positions.findByStrategyAndSymbolAndStatus(LaplacePaperExecutionService.STRATEGY,signal.symbol(),LaplacePositionStatus.OPEN);if(open.size()==1){PositionSide side=open.getFirst().getSide();if(execution.closeLongFastRegime(signal,open.getFirst())){writer.drain();return side;}}return null;
 }
