...
import com.crypto.laplace.service.LaplaceTrueSymbolStopLossCooldownService;
...
public class LaplacePaperTradeCoordinator {
 private final LaplaceStrategyProperties config;private final LaplacePaperPositionRepository positions;private final LaplacePaperExecutionService execution;private final LaplaceTradeJsonlWriter writer;private final LaplaceRuntimeService runtime;private final LaplaceDiagnosticLogService diagnostics;private final LaplaceTrueSymbolStopLossCooldownService symbolCooldown;private final ConcurrentHashMap<String,ReentrantLock> locks=new ConcurrentHashMap<>();private final ConcurrentHashMap<String,LaplaceSignal> rawStates=new ConcurrentHashMap<>();private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 public PositionSide mapRawSignalToExecutionSide(LaplaceSignal raw){return LaplaceDirectionMapper.mapDirection(LaplacePaperVariant.INVERTED_TRUE,raw);}
 ...
 public void onSignal(LaplaceSignalResult signal,boolean inEntryUniverse){
  ...
  if(!config.getLaplace().isPaperExecutionEnabled()||!config.getLaplace().getPaper().getInvertedTrue().isEnabled()){if(signal.entrySignal()!=LaplaceSignal.NONE&&paperDisabledLogged.compareAndSet(false,true))log.warn("LAPLACE_PAPER_EXECUTION_DISABLED variant=INVERTED_TRUE strategy={} symbol={}",LaplacePaperExecutionService.STRATEGY,signal.symbol());return;}
 ...
 private void coordinate(LaplaceSignalResult signal,boolean inEntryUniverse,PositionSide effective){
  ...
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(!inEntryUniverse){failure(signal,"FLAT","ENTRY","SYMBOL_OUTSIDE_ENTRY_UNIVERSE",null,false);return;}if(entryBlocked(signal,effective))return;try{execution.open(signal,effective,null,"FLAT");writer.drain();}catch(RuntimeException e){failure(signal,"FLAT","ENTRY",reason(e,"ENTRY_EXECUTION_FAILED"),e,true);}return;}
  ...
  boolean replacementBlocked=inEntryUniverse&&entryBlocked(signal,effective);
  try{execution.reverse(signal,p,effective,inEntryUniverse&&!replacementBlocked,replacementBlocked);writer.drain();}catch(RuntimeException e){failure(signal,p.getSide().name(),"REVERSAL",reason(e,"REVERSAL_CLOSE_FAILED"),e,true);}
 }
 private boolean entryBlocked(LaplaceSignalResult signal,PositionSide effective){double atrPercentage=signal.currentAtr14()/signal.signalCandleClose()*100.0;double slopeStrength=Math.abs(signal.currentNormalizedSlope());var cooldown=symbolCooldown.active(signal.symbol());if(cooldown.isPresent()){var active=cooldown.get();diagnostics.trueSymbolStopLossCooldown(signal,effective,atrPercentage,slopeStrength,active,symbolCooldown.remainingSeconds(active));log.info("LAPLACE_ENTRY_BLOCKED reason=TRUE_SYMBOL_STOP_LOSS_COOLDOWN symbol={} cooldownUntil={} signalTime={}",signal.symbol(),active.cooldownUntil(),signal.signalCandleCloseTime());return true;}var filter=config.getLaplace().getPaper().getInvertedTrue().getSoftFilter();boolean blocked=atrPercentage>=filter.getAtrMinPct()&&slopeStrength>=filter.getSlopeStrengthMin();if(blocked){diagnostics.trueSoftRiskFilter(signal,effective,atrPercentage,slopeStrength,filter.getAtrMinPct(),filter.getSlopeStrengthMin());log.info("LAPLACE_ENTRY_BLOCKED reason=TRUE_SOFT_RISK_FILTER symbol={} atrPercentage={} currentNormalizedSlope={} slopeStrength={} atrThreshold={} slopeStrengthThreshold={} signalTime={}",signal.symbol(),atrPercentage,signal.currentNormalizedSlope(),slopeStrength,filter.getAtrMinPct(),filter.getSlopeStrengthMin(),signal.signalCandleCloseTime());}return blocked;}
 ...
 public void clearRuntimeState(){rawStates.clear();locks.clear();paperDisabledLogged.set(false);}
}
