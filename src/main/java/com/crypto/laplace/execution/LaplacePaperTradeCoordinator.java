...
private final AtomicBoolean paperDisabledLogged=new AtomicBoolean(false);
 ...
 private void coordinate(LaplaceSignalResult signal,boolean poolActiveAfterTransition,PositionSide effective,boolean entryUniverseSnapshotContainsSymbol,Instant eligibilityEvaluatedAt){
  ...
  LaplaceMarketBreadthSnapshot breadth=breadthService==null?new LaplaceMarketBreadthSnapshot(signal.signalCandleCloseTime(),100,100,1,1,1,1):breadthService.snapshot(signal.signalCandleCloseTime());
  LaplaceEntryFilter.Decision decision=entryFilter.evaluate(signal,effective,breadth);
  if(open.isEmpty()){if(effective==null||!signal.eligibleForExecution())return;if(!decision.entryAllowed()){blockRiskyDirection(signal);writer.entrySkipped(signal.symbol(),signal.signalCandleCloseTime(),effective,java.math.BigDecimal.valueOf(signal.signalCandleClose()),decision.rejectionReason());decisionLog(signal,effective,breadth,decision,"SKIP",false);return;}decisionLog(signal,effective,breadth,decision,"OPEN_INVERTED",false);metrics.entryAttempts.incrementAndGet();if(!currentlyEligible){
  ...
  if(!decision.entryAllowed()){blockRiskyDirection(signal);writer.entrySkipped(signal.symbol(),signal.signalCandleCloseTime(),effective,java.math.BigDecimal.valueOf(signal.signalCandleClose()),decision.rejectionReason());decisionLog(signal,effective,breadth,decision,"SKIP",false);}else decisionLog(signal,effective,breadth,decision,"OPEN_INVERTED",false);
