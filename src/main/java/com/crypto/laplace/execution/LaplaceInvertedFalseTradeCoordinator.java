...
public class LaplaceInvertedFalseTradeCoordinator {
 ...
 public PositionSide mapRawSignalToExecutionSide(LaplaceSignal raw){return LaplaceDirectionMapper.mapDirection(LaplacePaperVariant.INVERTED_FALSE,raw);}
 ...
 public void onSignal(LaplaceSignalResult signal,boolean inEntryUniverse){
  ...
  if(!config.getLaplace().isPaperExecutionEnabled()||!config.getLaplace().getPaper().getInvertedFalse().isEnabled()){if(signal.entrySignal()!=LaplaceSignal.NONE&&paperDisabledLogged.compareAndSet(false,true))log.warn("LAPLACE_PAPER_EXECUTION_DISABLED variant=INVERTED_FALSE strategy={} symbol={}",LaplaceInvertedFalseExecutionService.STRATEGY,signal.symbol());return;}
 ...
 private String current(String symbol){var x=positions.findByStrategyAndSymbolAndStatus(LaplaceInvertedFalseExecutionService.STRATEGY,symbol,LaplacePositionStatus.OPEN);return x.isEmpty()?"FLAT":x.getFirst().getSide().name();}
 private void coordinate(LaplaceSignalResult signal,boolean inEntryUniverse,PositionSide effective){
  List<LaplaceInvertedFalsePositionEntity> open=positions.findByStrategyAndSymbolAndStatus(LaplaceInvertedFalseExecutionService.STRATEGY,signal.symbol(),LaplacePositionStatus.OPEN);if(open.size()>1){failure(signal,"CONFLICT","NONE","POSITION_STATE_CONFLICT",null,false);return;}
  ...
  LaplaceInvertedFalsePositionEntity p=open.getFirst();if(effective==null||p.getSide()==effective){log.info("LAPLACE_SAME_EFFECTIVE_SIDE_SIGNAL_IGNORED symbol={} side={}",signal.symbol(),p.getSide());return;}
