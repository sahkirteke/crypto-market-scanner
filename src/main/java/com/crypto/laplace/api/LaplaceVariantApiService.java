...
public class LaplaceVariantApiService {
 ...
 private View view(LaplaceInvertedFalsePositionEntity p){return new View(p.getId(),p.getSymbol(),p.getSide(),p.getStatus(),p.getEntryTime(),p.getEntryExecutionPrice(),p.getMargin(),p.getQuantity(),p.getNotional(),p.getLeverage(),p.getEntryFee(),p.getEntryFeeRate(),p.getExitTime(),p.getExitExecutionPrice(),p.getExitFee(),p.getNetPnl(),p.getExitReason());}
