...
public class LaplaceVariantApiService {
 private final LaplacePaperPositionRepository trueRepository;private final LaplaceInvertedFalsePositionRepository falseRepository;
 ...
 private Calculation calculate(LaplacePaperVariant variant){
  ...
  return new Calculation(new LaplaceOpenPaperPositionsResponse(responses,responses.size(),openPnl,closedPnl,openPnl,closedPnl.add(openPnl)),closed.size());
 }
 private List<View> load(LaplacePaperVariant variant,LaplacePositionStatus status){
  if(variant==LaplacePaperVariant.INVERTED_TRUE)return trueRepository.findByStrategyAndStatus(LaplacePaperExecutionService.STRATEGY,status).stream().map(this::view).toList();
  return falseRepository.findByStrategyAndStatus(LaplacePaperExecutionService.STRATEGY,status).stream().map(this::view).toList();
 }
 private LaplaceOpenPaperPositionResponse openResponse(View p,Map<String,BookTicker> tickers){BookTicker q=tickers.get(p.symbol());BigDecimal current=q==null?null:p.side()==PositionSide.LONG?q.getBidPrice():q.getAskPrice();if(current==null||current.signum()<=0)throw new IllegalStateException("EXECUTION_PRICE_UNAVAILABLE:"+p.symbol());return new LaplaceOpenPaperPositionResponse(p.id(),p.symbol(),p.side(),p.entryTime(),p.entryPrice(),current,pnl.priceMovePct(p.side(),p.entryPrice(),current),pnl.gross(p.side(),p.entryPrice(),current,p.quantity()),false);}
 ...
 private View view(LaplacePaperPositionEntity p){return new View(p.getId(),p.getSymbol(),p.getSide(),p.getStatus(),p.getEntryTime(),p.getEntryExecutionPrice(),p.getMargin(),p.getQuantity(),p.getNotional(),p.getLeverage(),p.getEntryFee(),p.getExitTime(),p.getExitExecutionPrice(),p.getExitFee(),p.getNetPnl(),p.getExitReason());}
 private View view(LaplaceInvertedFalsePositionEntity p){return new View(p.getId(),p.getSymbol(),p.getSide(),p.getStatus(),p.getEntryTime(),p.getEntryExecutionPrice(),p.getMargin(),p.getQuantity(),p.getNotional(),p.getLeverage(),p.getEntryFee(),p.getExitTime(),p.getExitExecutionPrice(),p.getExitFee(),p.getNetPnl(),p.getExitReason());}
