...
public class LaplaceDualSummaryService {
 ...
 private LaplaceVariantAnalysisSummaryResponse build(LaplacePaperVariant variant,List<View> open,List<View> closed,Map<String,BookTicker> tickers,Session session){
  ...
  long shockedExit=closed.stream().filter(x->"MARKET_SHOCK_PERSISTENT".equals(x.exitReason())).count();
 ...
 private View view(LaplacePaperPositionEntity x){return new View(x.getSessionId(),x.getSymbol(),x.getSide(),x.getEntryTime(),x.getExitTime(),x.getEntryExecutionPrice(),x.getQuantity(),x.getEntryFeeRate(),x.getEntryFee(),x.getExitFee(),x.getGrossPnl(),x.getNetPnl(),x.getNetPnlPct(),x.getExitReason());}
 ...
 private record View(String sessionId,String symbol,PositionSide side,Instant entryTime,Instant exitTime,BigDecimal entryPrice,BigDecimal quantity,BigDecimal feeRate,BigDecimal entryFee,BigDecimal exitFee,BigDecimal gross,BigDecimal net,BigDecimal netPct,String exitReason){}
