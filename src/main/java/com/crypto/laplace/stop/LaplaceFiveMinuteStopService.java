...
public class LaplaceFiveMinuteStopService {
    ...
    private boolean containsStop(StopPositionSnapshot s,List<Kline> candles,Instant now){BigDecimal stop=s.stopPrice()!=null?s.stopPrice():(s.side()==PositionSide.LONG?s.entryExecutionPrice().multiply(new BigDecimal("0.954")):s.entryExecutionPrice().multiply(new BigDecimal("1.046")));Instant entryOpen=entryCandleOpen(s.entryTime());return candles.stream().filter(c->c!=null&&c.getOpenTime()!=null&&c.getCloseTime()!=null&&!c.getCloseTime().isAfter(now)&&!Boolean.FALSE.equals(c.getClosed())&&c.getOpenTime().isAfter(entryOpen)&&(s.lastStopCheckedCandleOpenTime()==null||c.getOpenTime().isAfter(s.lastStopCheckedCandleOpenTime()))).anyMatch(c->s.side()==PositionSide.LONG?c.getLow().compareTo(stop)<=0:c.getHigh().compareTo(stop)>=0);}
