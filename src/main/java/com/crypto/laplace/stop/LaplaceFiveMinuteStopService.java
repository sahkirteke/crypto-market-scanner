...
public class LaplaceFiveMinuteStopService {
    ...
    public void catchUpOpenPositions(){
        ...
        for(StopPositionSnapshot initial:snapshots()){
            ...
            while(cursor.isBefore(now)){
                ...
                if(result.outcome()==LaplaceStopPersistenceService.Outcome.STOP_CLOSED||result.outcome()==LaplaceStopPersistenceService.Outcome.TIME_CLOSED){execution.drainOutbox();break;}
    ...
    private void classify(StopPositionSnapshot s,LaplaceStopPersistenceService.Result r,Set<String> closed,Set<String> advanced,Set<String> already){switch(r.outcome()){case STOP_CLOSED,TIME_CLOSED->closed.add(s.symbol());case CHECKPOINT_ADVANCED->advanced.add(s.symbol());case ALREADY_CLOSED->already.add(s.symbol());case NO_CHANGE->{}}}
    private boolean containsStop(StopPositionSnapshot s,List<Kline> candles,Instant now){BigDecimal stop=s.stopPrice()!=null?s.stopPrice():(s.side()==PositionSide.LONG?s.entryExecutionPrice().multiply(new BigDecimal("0.95")):s.entryExecutionPrice().multiply(new BigDecimal("1.05")));Instant entryOpen=entryCandleOpen(s.entryTime());return candles.stream().filter(c->c!=null&&c.getOpenTime()!=null&&c.getCloseTime()!=null&&!c.getCloseTime().isAfter(now)&&!Boolean.FALSE.equals(c.getClosed())&&c.getOpenTime().isAfter(entryOpen)&&(s.lastStopCheckedCandleOpenTime()==null||c.getOpenTime().isAfter(s.lastStopCheckedCandleOpenTime()))).anyMatch(c->s.side()==PositionSide.LONG?c.getLow().compareTo(stop)<=0:c.getHigh().compareTo(stop)>=0);}
