...
public class LaplaceRuntimeService implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        if (!properties.getLaplace().getPaper().getInvertedTrue().isEnabled()) return;
    ...
    public boolean isActive() {
        return properties.getLaplace().getPaper().getInvertedTrue().isEnabled()
                && current().getRuntimeState() == LaplaceRuntimeState.ACTIVE;
    }

    public boolean allowsMarketData() {
        if (!properties.getLaplace().getPaper().getInvertedTrue().isEnabled()) return false;
