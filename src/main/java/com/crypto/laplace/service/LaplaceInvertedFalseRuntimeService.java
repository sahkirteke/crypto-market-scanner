...
public class LaplaceInvertedFalseRuntimeService implements ApplicationRunner {
    ...
    public LaplaceInvertedFalseSessionEntity current() {
    ...
    public boolean beginLiquidation(BigDecimal realized, BigDecimal openPnl, BigDecimal exitFees,
        ...
        LaplaceInvertedFalseSessionEntity session = locked();
    ...
    @Transactional
    public LaplaceInvertedFalseSessionEntity activateNextSession() {
        LaplaceInvertedFalseSessionEntity previous = locked();
        if (previous.getRuntimeState() != LaplaceRuntimeState.INITIALIZING) throw new IllegalStateException("INVALID_RUNTIME_TRANSITION");
        LaplaceInvertedFalseSessionEntity next = LaplaceInvertedFalseSessionEntity.builder()
    ...
    public BigDecimal target(LaplaceInvertedFalseSessionEntity session) {
        return pct(session.getSessionStartCapital(), session.getProfitTargetPct());
    }

    public BigDecimal minimumLocked(LaplaceInvertedFalseSessionEntity session) {
        return pct(session.getSessionStartCapital(), session.getMinimumLockedProfitPct());
    }

    public LaplaceRuntimeStatusResponse status() {
        LaplaceInvertedFalseSessionEntity session = current();
    ...
    private LaplaceInvertedFalseSessionEntity initialSession() {
        var config = properties.getLaplace();
        return LaplaceInvertedFalseSessionEntity.builder().sessionId(UUID.randomUUID().toString())
    ...
    private LaplaceInvertedFalseSessionEntity locked() {
