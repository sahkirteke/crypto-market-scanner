...
public class StartupMarketUniverseService implements ApplicationRunner {
 ...
 public BigDecimal currentVolume(String symbol) {
  return client.getAll24hTickers().stream().filter(t->symbol.equals(t.getSymbol())).map(Ticker24h::getQuoteVolume).findFirst().orElse(null);
 }
