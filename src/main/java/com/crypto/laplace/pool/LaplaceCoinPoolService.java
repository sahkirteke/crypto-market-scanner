        ...
        try {
            ...
            for(String symbol:candidates) {
                ...
                if(entity!=null) {
                    ...
                    if(entity.getState()!=LaplaceCoinPoolState.PENDING_REMOVAL && entity.getState()!=LaplaceCoinPoolState.REMOVED && volume.compareTo(EXIT_THRESHOLD)<0)
                        transition(entity,open?LaplaceCoinPoolState.PENDING_REMOVAL:LaplaceCoinPoolState.REMOVED);
                    else pool.save(entity);
                }
                if((entity==null||entity.getState()==LaplaceCoinPoolState.REMOVED)&&volume.compareTo(ENTRY_THRESHOLD)>0) {
                ...
                List<String> failed=volume.compareTo(entity==null||entity.getState()==LaplaceCoinPoolState.REMOVED?ENTRY_THRESHOLD:EXIT_THRESHOLD)<0?List.of("MIN_VOLUME"):List.of();
