...
public class LaplaceCoinPoolService {
    ...
    public boolean isRiskyDirectionBlocked(String symbol,String rawSignal){return pool.findById(symbol).map(p->Boolean.TRUE.equals(p.getRiskyDirectionBlocked())&&rawSignal.equals(p.getBlockedRiskyRawSignal())).orElse(false);}
    public String blockedRiskyRawSignal(String symbol){return pool.findById(symbol).filter(p->Boolean.TRUE.equals(p.getRiskyDirectionBlocked())).map(LaplaceCoinPoolEntity::getBlockedRiskyRawSignal).orElse(null);}
    public boolean hasRiskyDirectionBlock(String symbol){return pool.findById(symbol).map(p->Boolean.TRUE.equals(p.getRiskyDirectionBlocked())).orElse(false);}
    @Transactional public void blockRiskyDirection(String symbol,String rawSignal,Instant candleClose){pool.findById(symbol).ifPresent(p->{p.setRiskyDirectionBlocked(true);p.setBlockedRiskyRawSignal(rawSignal);p.setBlockedRiskySignalCandleCloseTime(candleClose);pool.save(p);});}
    @Transactional public void clearRiskyDirectionBlock(String symbol){pool.findById(symbol).filter(p->Boolean.TRUE.equals(p.getRiskyDirectionBlocked())).ifPresent(p->{p.setRiskyDirectionBlocked(false);p.setBlockedRiskyRawSignal(null);p.setBlockedRiskySignalCandleCloseTime(null);pool.save(p);});}
    public boolean marketRetryMatches(String symbol,String rawSignal){return pool.findById(symbol).map(p->Boolean.TRUE.equals(p.getMarketRetryPending())&&rawSignal.equals(p.getPendingMarketRawSignal())).orElse(false);}
    public String pendingMarketRawSignal(String symbol){return pool.findById(symbol).filter(p->Boolean.TRUE.equals(p.getMarketRetryPending())).map(LaplaceCoinPoolEntity::getPendingMarketRawSignal).orElse(null);}
    public boolean marketRetryAlreadyRecorded(String symbol,String rawSignal,Instant candleClose){return pool.findById(symbol).map(p->Boolean.TRUE.equals(p.getMarketRetryPending())&&rawSignal.equals(p.getPendingMarketRawSignal())&&candleClose.equals(p.getPendingMarketSignalCandleCloseTime())).orElse(false);}
    @Transactional public void saveMarketRetry(String symbol,String rawSignal,String reason,Instant candleClose){pool.findById(symbol).ifPresent(p->{p.setMarketRetryPending(true);p.setPendingMarketRawSignal(rawSignal);p.setPendingMarketRejectionReason(reason);p.setPendingMarketSignalCandleCloseTime(candleClose);pool.save(p);});}
    @Transactional public void clearMarketRetry(String symbol){pool.findById(symbol).filter(p->Boolean.TRUE.equals(p.getMarketRetryPending())).ifPresent(p->{p.setMarketRetryPending(false);p.setPendingMarketRawSignal(null);p.setPendingMarketRejectionReason(null);p.setPendingMarketSignalCandleCloseTime(null);pool.save(p);});}
