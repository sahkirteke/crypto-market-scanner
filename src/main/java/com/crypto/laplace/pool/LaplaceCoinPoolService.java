...
public class LaplaceCoinPoolService {
    ...
    public boolean hasPendingRiskyEntry(String symbol,String rawSignal){return pool.findById(symbol).map(p->Boolean.TRUE.equals(p.getRiskyEntryPending())&&rawSignal.equals(p.getPendingRiskyRawSignal())).orElse(false);}
    public boolean hasAnyPendingRiskyEntry(String symbol){return pool.findById(symbol).map(p->Boolean.TRUE.equals(p.getRiskyEntryPending())).orElse(false);}
    @Transactional public void recordPendingRiskyEntry(String symbol,String rawSignal,Instant candleClose){pool.findById(symbol).ifPresent(p->{p.setRiskyEntryPending(true);p.setPendingRiskyRawSignal(rawSignal);p.setPendingRiskySignalCandleCloseTime(candleClose);pool.save(p);});}
    @Transactional public void clearPendingRiskyEntry(String symbol){pool.findById(symbol).filter(p->Boolean.TRUE.equals(p.getRiskyEntryPending())).ifPresent(p->{p.setRiskyEntryPending(false);p.setPendingRiskyRawSignal(null);p.setPendingRiskySignalCandleCloseTime(null);pool.save(p);});}
