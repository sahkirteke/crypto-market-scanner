...
class LaplaceSignalServiceTest {
 @Test void confirmedLongAndStrongReversalOnlyAfterWarmup(){assertSignal(101,100,99,10,10,2,LaplaceSignal.LONG,LaplaceSignal.LONG);}
 @Test void oneNewBarCannotSignal(){assertSignal(101,100,99,10,10,1,LaplaceSignal.NONE,LaplaceSignal.NONE);}
 @Test void confirmedShortAndStrongReversal(){assertSignal(99,100,101,10,10,2,LaplaceSignal.SHORT,LaplaceSignal.SHORT);}
 ...
 private void assertSignal(double current,double previous,double two,double ca,double pa,int bars,LaplaceSignal entry,LaplaceSignal reversal){
  ...
  var result=new LaplaceSignalService(kr,atr).calculate("BTCUSDT",candles,bars);assertThat(result.entrySignal()).isEqualTo(entry);assertThat(result.strongReversalSignal()).isEqualTo(reversal);assertThat(result.eligibleForExecution()).isEqualTo(bars>=2&&entry!=LaplaceSignal.NONE);
 }
}
