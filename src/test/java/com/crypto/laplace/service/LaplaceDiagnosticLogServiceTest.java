...
class LaplaceDiagnosticLogServiceTest {
 @Test void signalsAreLoggedOnlyToTrueDirectory(){var jsonl=mock(JsonlDecisionLogService.class);var service=new LaplaceDiagnosticLogService(jsonl,new LaplaceStrategyProperties(),new LaplaceDirectionMapper(),Clock.systemUTC());service.signal(signal());verify(jsonl).append(eq("signals/laplace/inverted-true/diagnostics"),eq("laplace-signals"),anyMap());verifyNoMoreInteractions(jsonl);}
