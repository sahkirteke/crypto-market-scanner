...
class LaplaceVolumeProfileServiceTest {
 ...
 @Test void completedSessionProfileIsCachedButCurrentDeltaIsReloadedPerSignal(){BinanceFuturesClient c=mock(BinanceFuturesClient.class);when(c.getKlines(anyString(),eq("5m"),eq(78),any())).thenReturn(sessionCandles(false));when(c.getKlines(anyString(),eq("5m"),eq(20),any())).thenReturn(recent(true));LaplaceVolumeProfileService service=new LaplaceVolumeProfileService(c);service.evaluate("BTCUSDT",signalTime(),99,PositionSide.LONG);service.evaluate("BTCUSDT",signalTime().plusSeconds(1800),99,PositionSide.LONG);verify(c,times(1)).getKlines(eq("BTCUSDT"),eq("5m"),eq(78),any());verify(c,times(2)).getKlines(eq("BTCUSDT"),eq("5m"),eq(20),any());}
 @Test void exactlySeventySevenClosedSessionCandlesAreFailClosed(){BinanceFuturesClient c=mock(BinanceFuturesClient.class);when(c.getKlines(anyString(),eq("5m"),eq(78),any())).thenReturn(sessionCandles(false).subList(0,77));assertThat(new LaplaceVolumeProfileService(c).evaluate("BTCUSDT",signalTime(),102,PositionSide.LONG).rejectionReason()).isEqualTo(VolumeProfileRejectionReason.VP_PROFILE_NOT_AVAILABLE);}
 @Test void startupPrewarmRequestsExactlySeventyEightSessionAndTwelveCurrentCandles(){BinanceFuturesClient c=mock(BinanceFuturesClient.class);when(c.getKlines("BTCUSDT","5m",78,Instant.parse("2026-07-30T13:30:00Z"))).thenReturn(sessionCandles(false));when(c.getKlines("BTCUSDT","5m",12)).thenReturn(recent(true));var result=new LaplaceVolumeProfileService(c).prewarm("BTCUSDT",signalTime());assertThat(result.loadedSessionCandleCount()).isEqualTo(78);assertThat(result.loadedCurrentCandleCount()).isEqualTo(12);}
 @Test void startupPrewarmFailsWhenCurrentRequestContainsOpenCandle(){BinanceFuturesClient c=mock(BinanceFuturesClient.class);when(c.getKlines(anyString(),eq("5m"),eq(78),any())).thenReturn(sessionCandles(false));List<Kline> current=new ArrayList<>(recent(true));current.set(11,candle(signalTime(),100,101,100,70));when(c.getKlines("BTCUSDT","5m",12)).thenReturn(current);org.assertj.core.api.Assertions.assertThatThrownBy(()->new LaplaceVolumeProfileService(c).prewarm("BTCUSDT",signalTime())).hasMessage("VP_INSUFFICIENT_60M_DATA");}
 @Test void prewarmReusesSameSessionButLoadsNewSessionOnce(){BinanceFuturesClient c=mock(BinanceFuturesClient.class);when(c.getKlines(anyString(),eq("5m"),eq(78),any())).thenReturn(sessionCandles(false));when(c.getKlines(anyString(),eq("5m"),eq(12))).thenReturn(recent(true));LaplaceVolumeProfileService service=new LaplaceVolumeProfileService(c);service.prewarm("BTCUSDT",signalTime());service.prewarm("BTCUSDT",signalTime().plusSeconds(1800));verify(c,times(1)).getKlines(eq("BTCUSDT"),eq("5m"),eq(78),any());verify(c,times(2)).getKlines("BTCUSDT","5m",12);}
