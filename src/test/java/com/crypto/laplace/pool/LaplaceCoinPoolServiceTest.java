...
class LaplaceCoinPoolServiceTest {
 ...
 @Test void persistentReentryLockSurvivesServiceRecreationAndOnlyOppositeUnlocks(){BinanceFuturesClient client=mock(BinanceFuturesClient.class);LaplaceCoinPoolRepository repository=mock(LaplaceCoinPoolRepository.class);LaplaceCoinPoolEntity row=LaplaceCoinPoolEntity.builder().symbol("BTCUSDT").state(LaplaceCoinPoolState.ACTIVE).build();when(repository.findById("BTCUSDT")).thenReturn(java.util.Optional.of(row));LaplaceCoinPoolService first=new LaplaceCoinPoolService(client,repository,mock(LaplacePaperPositionRepository.class),mock(LaplaceCapitalSnapshotRepository.class));first.recordStop("BTCUSDT","LONG","SHORT","p1",java.time.Instant.now());LaplaceCoinPoolService restarted=new LaplaceCoinPoolService(client,repository,mock(LaplacePaperPositionRepository.class),mock(LaplaceCapitalSnapshotRepository.class));assertThat(restarted.allowAfterOppositeSignal("BTCUSDT","LONG")).isFalse();assertThat(restarted.allowAfterOppositeSignal("BTCUSDT","NONE")).isFalse();assertThat(restarted.allowAfterOppositeSignal("BTCUSDT","LONG")).isFalse();assertThat(restarted.allowAfterOppositeSignal("BTCUSDT","SHORT")).isTrue();assertThat(row.getUnblockedBySignalSide()).isEqualTo("SHORT");}
}
