...
class LaplaceFiveMinuteStopServiceTest {
 ...
 @Test void historicalGapAndTouchPricesRemainDeterministic(){LaplacePaperPositionEntity p=position();p.setStopPrice(new BigDecimal("94.5"));Kline gap=candle("95","90");gap.setOpen(new BigDecimal("93"));assertThat(LaplaceFiveMinuteStopService.historicalExecutionPrice(p,gap)).isEqualByComparingTo("93");Kline touch=candle("100","94.5");assertThat(LaplaceFiveMinuteStopService.historicalExecutionPrice(p,touch)).isEqualByComparingTo("94.5");}
 private LaplacePaperPositionEntity position(){return LaplacePaperPositionEntity.builder().id("position").strategy(LaplacePaperExecutionService.STRATEGY).symbol("BTCUSDT").side(PositionSide.LONG).status(LaplacePositionStatus.OPEN).entryExecutionPrice(new BigDecimal("100")).entryTime(Instant.now().minusSeconds(1800)).stopPrice(new BigDecimal("94.5")).build();}
