...
class LaplaceStopPersistenceServiceTest {
 @Test void checkpointAdvancesMonotonicallyOnFreshManagedEntityWithoutMerge(){Fixture f=new Fixture();Instant ten=Instant.parse("2026-07-27T10:00:00Z"),tenFive=ten.plusSeconds(300);f.p.setLastStopCheckedCandleOpenTime(tenFive);var stale=new LaplaceFiveMinuteStopService.StopPositionSnapshot("p","BTCUSDT",PositionSide.LONG,new BigDecimal("100"),new BigDecimal("95.4"),ten.minusSeconds(3600),ten);var result=f.service.persist(stale,List.of(candle(ten.plusSeconds(120),"99")),ten.plusSeconds(600));assertThat(result.outcome()).isEqualTo(LaplaceStopPersistenceService.Outcome.NO_CHANGE);assertThat(f.p.getLastStopCheckedCandleOpenTime()).isEqualTo(tenFive);verify(f.repo,never()).save(any());}
 ...
 @Test void stopTouchClosesOnceAdvancesCheckpointAndPersistsReentryLock(){Fixture f=new Fixture();Instant open=Instant.now().minusSeconds(600);Kline c=candle(open,"95.4");when(f.execution.closeByStop(eq("p"),eq(c),eq(new BigDecimal("95.4")),eq("FIVE_MINUTE_STOP_SIMULATION"))).thenReturn(Optional.of(f.p));var result=f.service.persist(f.snapshot(),List.of(c),Instant.now());assertThat(result.outcome()).isEqualTo(LaplaceStopPersistenceService.Outcome.STOP_CLOSED);assertThat(f.p.getLastStopCheckedCandleOpenTime()).isEqualTo(open);verify(f.execution,times(1)).closeByStop(any(),any(),any(),any());verify(f.coordinator).onStopLossClosed(eq("BTCUSDT"),eq("LONG"),eq("LONG"),eq("p"),any());}
 ...
 private static class Fixture{final LaplacePaperPositionRepository repo=mock(LaplacePaperPositionRepository.class);final LaplacePaperExecutionService execution=mock(LaplacePaperExecutionService.class);final LaplacePaperTradeCoordinator coordinator=mock(LaplacePaperTradeCoordinator.class);final LaplacePaperPositionEntity p=LaplacePaperPositionEntity.builder().id("p").symbol("BTCUSDT").side(PositionSide.LONG).status(LaplacePositionStatus.OPEN).entryRawSignal("LONG").entryExecutionPrice(new BigDecimal("100")).entryTime(Instant.now().minusSeconds(3600)).stopPrice(new BigDecimal("95.4")).exitTime(Instant.now()).build();final LaplaceStopPersistenceService service=new LaplaceStopPersistenceService(repo,execution,coordinator);Fixture(){when(repo.findByIdForUpdate("p")).thenReturn(Optional.of(p));}LaplaceFiveMinuteStopService.StopPositionSnapshot snapshot(){return LaplaceFiveMinuteStopService.StopPositionSnapshot.from(p);}}
}
